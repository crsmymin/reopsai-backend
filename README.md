...
│
├── routes/                   # 기존 API Blueprints (호환 controller 유지)
│   ├── auth.py              # 인증/권한 controller
│   ├── workspace.py         # 프로젝트/스터디/아티팩트 controller
│   ├── screener.py          # 참여자 선별 controller
│   ├── plan_routes.py       # 리서치 계획서 AI 생성 controller
│   ├── survey_routes.py     # 설문지 AI 생성/관리 controller
│   ├── guideline_routes.py  # 가이드라인 AI 생성 controller
│   ├── study.py             # 스터디 slug 조회 controller
│   ├── generator.py         # 문서 업로드/생성 controller
│   ├── artifact_ai.py       # AI 아티팩트 편집 controller
│   ├── ai_persona.py        # AI 페르소나 시뮬레이션
│   ├── b2b.py               # B2B 기능 controller
│   └── admin.py             # 관리자 기능 controller
│
├── db/                      # 데이터베이스 계층
...
- `routes/*`는 요청 파싱, JWT/context 조회, 응답 JSON/status mapping에 집중하는 controller 역할을 맡습니다.
- `reopsai_backend.application/*_service.py`는 workspace, auth, b2b, admin, plan/survey/guideline, artifact AI, screener 등 use case orchestration을 담당합니다.
...
|------|------------|---------|------------|
| Workspace CRUD/AI | `routes/workspace.py` | `workspace_service.py`, `workspace_ai_service.py` | `workspace_repository.py` |
| Auth | `routes/auth.py` | `auth_service.py` | `auth_repository.py` |
| B2B | `routes/b2b.py` | `b2b_service.py` | `b2b_repository.py` |
| Admin | `routes/admin.py` | `admin_service.py`, `admin_usage_service.py`, `admin_backoffice_service.py` | `admin_repository.py`, `admin_usage_repository.py`, `admin_backoffice_repository.py` |
| AI 생성 | `routes/plan_routes.py`, `routes/survey_routes.py`, `routes/guideline_routes.py` | `plan_service.py`, `plan_generation_service.py`, `survey_service.py`, `guideline_service.py` | `plan_repository.py`, `survey_repository.py`, `guideline_repository.py` |
| Artifact AI | `routes/artifact_ai.py` | `artifact_ai_service.py` | `artifact_ai_repository.py` |
| 기타 route | `routes/screener.py`, `routes/study.py`, `routes/demo.py`, `routes/dev_evaluator.py`, `routes/generator.py` | 각 `*_service.py` | 필요 시 각 `*_repository.py` |
