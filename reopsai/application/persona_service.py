...
class PersonaService:
    ...
    def _resolve_run_personas(self, db_session, *, company_id: int, user_id: int, explicit_ids=None, source_data=None):
        ...
        if persona_ids:
            return self.repository.list_personas_by_ids(db_session, company_id=company_id, user_id=user_id, persona_ids=persona_ids)
        return self.repository.list_all_personas(db_session, company_id=company_id, user_id=user_id)
...
""".strip()
    ...
    def _run_interview_for_persona_id(
        ...
        *,
        ...
        with self.session_factory() as db_session:
            personas = self._resolve_run_personas(db_session, company_id=company_id, user_id=user_id, explicit_ids=[persona_id])
    ...
    def folder_payload(self, folder, *, persona_count=None):
        payload = {
            "id": folder.id,
        ...
        if persona_count is not None:
            payload["persona_count"] = int(persona_count or 0)
            payload["personaCount"] = int(persona_count or 0)
        return payload
    ...
    def list_folders(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            folders = self.repository.list_folders(db_session, company_id=company_id, user_id=user_id)
            persona_counts = self.repository.count_visible_personas_by_folder_ids(
                db_session,
                company_id=company_id,
                user_id=user_id,
            ...
            return self._ok(
                {
                    "data": [
                        self.folder_payload(row, persona_count=persona_counts.get(int(row.id), 0))
                        for row in folders
                    ]
                }
            )
    ...
    def create_folder(self, *, company_id: int, user_id: int, data: dict):
        ...
        with self.session_factory() as db_session:
            ...
            try:
                ...
                db_session.rollback()
                return self._error("duplicate", "이미 존재하는 폴더명입니다.", 409)
            return self._ok({"data": self.folder_payload(folder, persona_count=0)}, 201)
    ...
    def delete_folder(self, *, company_id: int, user_id: int, folder_id: int):
        with self.session_factory() as db_session:
            ...
            if getattr(folder, "is_default", False):
                return self._error("forbidden", "default folder cannot be deleted", 403)
            if not self._can_modify(db_session, folder, company_id=company_id, user_id=user_id):
                return self._error("forbidden", "insufficient permissions", 403)
            self.repository.soft_delete_folder(db_session, folder, user_id=user_id)
            return self._ok()

    def list_personas(self, *, company_id: int, user_id: int, page: int, limit: int, search=None, folder_id=None, no_folder=False):
        with self.session_factory() as db_session:
    ...
    def create_persona(self, *, company_id: int, user_id: int, data: dict):
        ...
        if not self._require_name(data):
            return self._error("invalid", "name is required", 400)
        with self.session_factory() as db_session:
            if data.get("folder_id") and not self.repository.get_visible_folder(db_session, company_id=company_id, user_id=user_id, folder_id=data["folder_id"]):
    ...
    def save_generated_personas(self, *, company_id: int, user_id: int, data: dict):
        ...
        created = []
        with self.session_factory() as db_session:
            if folder_id and not self.repository.get_visible_folder(db_session, company_id=company_id, user_id=user_id, folder_id=folder_id):
                return self._error("invalid", "folder not found", 400)
    ...
    def get_persona(self, *, company_id: int, user_id: int, persona_id: int):
        with self.session_factory() as db_session:
            persona = self.repository.get_persona(db_session, company_id=company_id, user_id=user_id, persona_id=persona_id)
            if not persona:
    ...
    def update_persona(self, *, company_id: int, user_id: int, persona_id: int, data: dict):
        with self.session_factory() as db_session:
            ...
            if "folder_id" in data:
                ...
                current_folder_id = getattr(persona, "folder_id", None)
                folder_changed = str(target_folder_id or "") != str(current_folder_id or "")
                if folder_changed:
                    if current_folder_id:
                        current_folder = self.repository.get_folder(db_session, company_id=company_id, folder_id=current_folder_id)
                        if getattr(current_folder, "is_default", False):
                            return self._error("forbidden", "default folder personas cannot be moved", 403)
                    if target_folder_id:
                        target_folder = self.repository.get_visible_folder(
                            db_session,
                            company_id=company_id,
                            user_id=user_id,
                        ...
                        if not target_folder:
                            return self._error("invalid", "folder not found", 400)
                        if getattr(target_folder, "is_default", False):
                            return self._error("forbidden", "personas cannot be moved into default folders", 403)
    ...
    def get_memory(self, *, company_id: int, user_id: int, persona_id: int):
        with self.session_factory() as db_session:
            persona = self.repository.get_persona(db_session, company_id=company_id, user_id=user_id, persona_id=persona_id)
            if not persona:
                return self._error("not_found", "persona not found", 404)
    ...
    def list_ui_tests(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            return self._ok({"data": [self.ui_test_payload(row) for row in self.repository.list_ui_tests(db_session, company_id=company_id, user_id=user_id)]})

    def get_ui_test(self, *, company_id: int, test_id: int, user_id: int | None = None):
        with self.session_factory() as db_session:
            test = self.repository.get_ui_test(db_session, company_id=company_id, test_id=test_id, user_id=user_id)
            if not test:
    ...
    def run_ui_test(self, *, company_id: int, user_id: int, test_id: int, data: dict):
        with self.session_factory() as db_session:
            ...
            personas = self._resolve_run_personas(db_session, company_id=company_id, user_id=user_id, explicit_ids=data.get("persona_ids") or data.get("personaIds"), source_data=source_data)
            if not personas:
    ...
    def list_ui_results(self, *, company_id: int, user_id: int, test_id: int):
        with self.session_factory() as db_session:
            if not self.repository.get_ui_test(db_session, company_id=company_id, user_id=user_id, test_id=test_id):
                return self._error("not_found", "test not found", 404)
            rows = self.repository.list_ui_test_results(db_session, company_id=company_id, test_id=test_id)
    ...
    def list_ab_tests(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            return self._ok({"data": [self.ab_test_payload(row) for row in self.repository.list_ab_tests(db_session, company_id=company_id, user_id=user_id)]})

    def get_ab_test(self, *, company_id: int, user_id: int, ab_test_id: int):
        with self.session_factory() as db_session:
            test = self.repository.get_ab_test(db_session, company_id=company_id, user_id=user_id, ab_test_id=ab_test_id)
            if not test:
    ...
    def list_ab_results(self, *, company_id: int, user_id: int, ab_test_id: int):
        with self.session_factory() as db_session:
            if not self.repository.get_ab_test(db_session, company_id=company_id, user_id=user_id, ab_test_id=ab_test_id):
                return self._error("not_found", "ab test not found", 404)
            rows = self.repository.list_ab_test_results(db_session, company_id=company_id, ab_test_id=ab_test_id)
            return self._ok({"data": [self.ab_result_payload(row) for row in rows]})

    def list_combined_tests(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            ui_items = [
                {
                    "id": row.id,
                ...
                for row in self.repository.list_ui_tests(db_session, company_id=company_id, user_id=user_id)
            ]
            ab_items = [
                {
                    "id": row.id,
                ...
                for row in self.repository.list_ab_tests(db_session, company_id=company_id, user_id=user_id)
            ]
            interview_items = [
                {
                    "id": row.id,
                ...
                for row in self.repository.list_interviews(db_session, company_id=company_id, user_id=user_id)
            ]
    ...
    def list_interview_personas(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            return self._ok({"data": [self.persona_payload(row) for row in self.repository.list_all_personas(db_session, company_id=company_id, user_id=user_id)]})
    ...
    def list_interviews(self, *, company_id: int, user_id: int):
        with self.session_factory() as db_session:
            payloads = []
            for row in self.repository.list_interviews(db_session, company_id=company_id, user_id=user_id):
    ...
    def get_interview(self, *, company_id: int, user_id: int, interview_id: int):
        with self.session_factory() as db_session:
            interview = self.repository.get_interview(db_session, company_id=company_id, user_id=user_id, interview_id=interview_id)
            if not interview:
    ...
    def run_interview(self, *, company_id: int, user_id: int, interview_id: int, data: dict):
        with self.session_factory() as db_session:
            ...
            personas = self._resolve_run_personas(db_session, company_id=company_id, user_id=user_id, explicit_ids=persona_ids)
            if not personas:
    ...
    def sync_figma_file(self, *, company_id: int, user_id: int, data: dict):
        ...
        with self.session_factory() as db_session:
            ...
            if not flows:
                return self._error("missing_flow", "파일 내 프로토타입 Flow가 연결되어 있는지 확인해주세요.", 400)
