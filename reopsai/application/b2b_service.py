...
from reopsai.infrastructure.repositories import B2bRepository, BUSINESS_ACCOUNT_TYPE, DEFAULT_BUSINESS_PASSWORD
    ...
    def remove_team_member(self, *, user_id: int, company_id_claim: Optional[int], member_user_id: int) -> B2bResult:
        with self.session_factory() as db_session:
            ...
            if not target_user:
                ...
                return B2bResult("ok", {"deleted_account": False})
            if (target_user.tier or "").strip().lower() == "super":
                return B2bResult("super_account")
            affected = self.repository.delete_member_account(db_session, member_user_id=member_user_id)
            return B2bResult("ok", {"deleted_account": True, "affected": affected})
